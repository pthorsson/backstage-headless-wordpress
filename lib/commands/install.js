...
module.exports = () => inquiries().then(() => installer([
    ...
    () => {
        ...
        utils.log('  1. Run "node hlwp -s"', { type: 'blank' });
        utils.log('  2. Go to localhost:8080/wp-admin', { type: 'blank' });
        utils.log(`  3. Login with ${data.wordpress.username} / ${data.wordpress.password}`, { type: 'blank' });
    }
