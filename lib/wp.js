...
let wp = {
    ...
    tasks: {
        ...
        install: () => (next, exit) => {
            ...
            wpcli(`core install --url="${data.wordpress.url}" --title="${data.theme.name}" --admin_user="${data.wordpress.username}" --admin_password="${data.wordpress.password}" --admin_email="${data.wordpress.email}" --skip-email`)
                .then(() => wpcli(`option update home "${data.wordpress.url}"`), () => {
                    utils.log('Could not install WordPress', { type: 'error'});
                    exit();
                })
                .then(() => wpcli(`option update siteurl "${data.wordpress.url}"`))
        ...
        addTestData: () => (next, exit) => {
            ...
            wpcli(`option update blogdescription "${data.theme.desc}"`)
                .then(() => wpcli(`post update "1" wordpress/wp-content/themes/${data.theme.dir}/post-content/sample-post.txt --post_title="Sample Post" --post_name=sample-post`))
                .then(() => wpcli(`post create wordpress/wp-content/themes/${data.theme.dir}/post-content/welcome.txt --post_type=page --post_status=publish --post_name=welcome --post_title="Congratulations!"`))
